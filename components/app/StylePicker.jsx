...
export default function StylePicker ({ open, onClose, settings, set, tier = 'free', trying, onTry, onLocked }) {
            ...
            <div className="styles">
              {STYLE_LIST.map(st => {
                ...
                return (
                  <button
                    key={st.id}
                    className={`style${(trying || settings.style) === st.id ? ' on' : ''}${isLocked ? ' locked' : ''}`}
                    ...
                    onClick={() => {
                      if (!isLocked) { onTry?.(null); return set('style', st.id) }
                      // Wear it on the real slides so the choice is informed.
                      onTry?.(trying === st.id ? null : st.id)
                    }}
                  ...
                  >
                    ...
                    <span className="style-text">
                      <strong>
                        {st.name}
                        {isLocked && <em>{trying === st.id ? 'Previewing' : (TIER_DETAIL[needs(st)]?.name || 'Plus')}</em>}
                      </strong>
                      <span>{st.blurb}</span>
                    </span>
                  </button>
