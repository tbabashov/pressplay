...
// surfaces, its dividers, its ink — and its layout. That last one is the point:
// a style that only recolours the same arrangement is the same slide in a
// different jumper, which is exactly what these were. The layout key is what
// the frames branch on to put the pieces somewhere else entirely.
...
export const STYLES = {
  ...
  signature: {
    ...
    // No font of its own on purpose: it falls through to FONT, the system
    // stack the approved slides in public/slides were rendered with. DESIGN.md
    // is explicit that this look is signed off and not to be redesigned.
    dome: true,
    type: { displayWeight: 800, displayTrack: '-1px', labelTrack: '0.14em', labelCase: 'uppercase', labelWeight: 800, coverRadius: '26px', chipRadius: '999px' },
    looks: ['gradient', 'glass', 'dome'],
    name: 'Press Play',
  ...
  paper: {
    ...
    font: "var(--font-serif), 'Fraunces', Georgia, serif",
    dome: false,
    type: { displayWeight: 800, displayTrack: '-0.02em', labelTrack: '0.16em', labelCase: 'uppercase', labelWeight: 700, coverRadius: '0px',  chipRadius: '0px' },
    looks: [],
    name: 'Paper',
    ...
    rule: () => '1px solid rgba(var(--ink-rgb), 0.26)'
  },
  ...
  marquee: {
    ...
    dome: false,
    type: { displayWeight: 900, displayTrack: '-2.4px', labelTrack: '0.2em', labelCase: 'uppercase', labelWeight: 900, coverRadius: '0px',  chipRadius: '2px' },
    looks: [],
    name: 'Marquee',
    blurb: 'Gig poster. The record behind everything, in hard-edged blocks.',
    // A style can bring settings with it. Marquee without the cover behind it
    // is a poster for nothing.
    defaults: { background: 'cover', backgroundDim: 0.58 },
    tier: 'free',
    ink: '#ffffff',
    inkRgb: '255, 255, 255',
    bg: palette => {
  ...
  mono: {
    ...
    font: "var(--font-mono), 'Space Grotesk', ui-sans-serif, sans-serif",
    dome: false,
    type: { displayWeight: 700, displayTrack: '-0.02em', labelTrack: '0.2em', labelCase: 'uppercase', labelWeight: 600, coverRadius: '0px',  chipRadius: '3px' },
    looks: [],
    name: 'Mono',
  ...
  aurora: {
    ...
    font: "var(--font-glow), 'Outfit', -apple-system, sans-serif",
    // No dome. With one the slide was the signature card with the lights
    // turned up, which is exactly what it looked like.
    dome: false,
    type: { displayWeight: 600, displayTrack: '-0.03em', labelTrack: '0.22em',  labelCase: 'uppercase', labelWeight: 500, coverRadius: '50%', chipRadius: '999px' },
    looks: [],
    name: 'Aurora',
...
// Explicit rather than object order: the free styles lead, then Aurora, then
// Press Play. Insertion order put the subscription-only one first, which is a
// locked card at the top of the list for anyone who has not paid.
export const STYLE_ORDER = ['paper', 'marquee', 'mono', 'aurora', 'signature']
export const STYLE_LIST = STYLE_ORDER.map(id => STYLES[id]).filter(Boolean)
export const styleOf = theme => STYLES[theme?.style] || STYLES.paper
export const styleHas = (theme, look) => (styleOf(theme).looks || []).includes(look)
