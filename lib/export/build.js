...
export async function buildExport (email, albumId, opts = {}) {
  ...
  return {
    ...
    // The whole ladder, not only how tall it is. The slides need its precision
    // too, so a rating set to two decimals reads 8.44 on the post as well as on
    // the screen it was typed into.
    scale: mine?.scaleModel ?? null
  }
}
