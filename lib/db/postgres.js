...
// Every table this file creates has to be named here. The check is what stops
// the DDL running on each cold start, and a table left out of it is a table a
// database that already has the others will never get: adding one means adding
// it to this list too. The DDL itself is all "if not exists", so a database
// missing only the new table runs it once and stops.
const TABLES = ['reviews', 'preferences', 'profiles', 'comments', 'follows',
  'snapshots', 'discography', 'credentials', 'votes']

const schemaPresent = async p => {
  const { rows } = await p.query(
    `select ${TABLES.map(t => `to_regclass('public.${t}') is not null`).join(' and ')} as ok`)
