...
// The file store reads the whole file either way, so there is nothing to leave
// behind. It returns the full rows and the shapes fall back to the snapshot,
// which keeps local and production rendering identical.
...
export async function markSubscriptionCancelled (email, { status, endsAt }) {
  return mutate(data => {
    ...
    data[k] = {
      ...
      subscriptionEndsAt: endsAt || was.subscriptionEndsAt || was.subscriptionRenewsAt || null,
