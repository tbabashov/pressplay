...
import { normaliseDecimal } from './rating-scale.js'
...
// What a criterion is worth, or null when it was never answered.
//
// Number('') is 0 and so is Number(null), so the obvious Number.isFinite check
// turns "left blank" into the harshest score on the scale — a criterion nobody
// filled in was being published as a zero. Blank, null and undefined are all
// "no answer"; only an actual number counts.
export const criterionValue = raw => {
  if (raw === '' || raw === null || raw === undefined) return null
  const n = Number(normaliseDecimal(raw))
...
export const SUPERLATIVES = [
  ...
  { key: 'thoughts', label: 'Personal thoughts', kind: 'text',
    note: 'Whatever you want to say about it, in your own words.' },
...
// The artist half of an album key on its own, so anything scoping a list of
// keys to one artist normalises the name exactly the way the key did.
export const artistKey = a => String(a ?? '').toLowerCase().trim()

export const albumKey = (artist, name) =>
  `${artistKey(artist)}::` +
  `${String(name ?? '').toLowerCase().trim().replace(/\s*[([].*$/, '')}`

// The hidden keys belonging to a given artist or artists. Hidden albums are
// stored as one list for the whole account, so any screen showing a single
// record has to narrow it: unnarrowed, a Drake export listed a Freddie Gibbs
// album under "taken off the slides" and offered to put it back.
export function hiddenForArtist (hidden, artists) {
  const here = new Set(
    (Array.isArray(artists) ? artists : [artists]).filter(Boolean).map(artistKey))
  if (!here.size) return []
  return normaliseHidden(hidden).filter(k => here.has(k.split('::')[0]))
}
...
// The longest a typed superlative may be. Long enough for a paragraph, short
// enough that it cannot be used to store something else in the review.
export const TEXT_SUPERLATIVE_MAX = 600

// Selections used to be saved exactly as they arrived, which was survivable
// while every one of them was a track title picked from a list on the page.
// A field somebody types into is not that, so this bounds what can be stored:
// keys nobody defined are dropped, and text is trimmed and cut to length.
export function normaliseSelections (raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {}
  const out = {}
  for (const [key, value] of Object.entries(raw)) {
    const spec = superlativeByKey[key]
    if (!spec || value === null || value === undefined) continue
    const text = String(value)
    if (!text.trim()) continue
    out[key] = spec.kind === 'text'
      // Only the newlines someone typed, and no runs of them: a slide has a
      // fixed amount of room and cannot honour twenty blank lines.
      ? text.replace(/\r\n?/g, '\n').replace(/\n{3,}/g, '\n\n').trim().slice(0, TEXT_SUPERLATIVE_MAX)
      : text.slice(0, 200)
  }
  return out
}
