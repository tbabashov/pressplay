...
export const TIER_DETAIL = {
  free: {
    ...
    perks: [
      ...
      'One artist cut-out',
      'The ten point scale and the built in criteria',
...
// Whether today's allowance leaves room to build a record's slides.
//
// The rule has two halves and the second is the one that is easy to drop: an
// album already built today can be built again for nothing, because the limit
// counts records put out and re-exporting one after fixing a typo is not a
// second record. The export screen promises this in as many words, so a caller
// that only asks whether used < cap contradicts the product.
//
// Lives here rather than at the two call sites so the export route and the
// button that points at it cannot disagree about who is allowed through.
export const canBuildSlides = ({ cap, used, builtToday }) =>
  cap === Infinity || builtToday === true || used < cap
...
// What a year actually saves against paying by the month, worked out from the
// two prices rather than written down beside them: a discount quoted as a
// number someone typed is a discount that goes stale the first time a price
// moves.
export function yearlySaving (tier) {
  const t = TIER_DETAIL[TIERS.includes(tier) ? tier : 'free']
  if (!t.monthly || !t.yearly) return null
  const full = t.monthly * 12
  const off = Math.round((1 - t.yearly / full) * 100)
  return off > 0 ? { percent: off, saves: Math.round((full - t.yearly) * 100) / 100, full } : null
}
