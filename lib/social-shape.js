...
import { DEFAULT_CRITERIA, criterionValue } from './preferences.js'
import { isVerified } from './profile.js'
...
export const publicCard = r => ({
  ...
  // Computed by a lite read, or counted off the snapshot when there is one.
...
export const publicPost = r => {
  ...
  // Resolved, not raw. Leaving best song on automatic stores an empty string,
  // so a card reading r.selections straight showed nothing where the published
  // page and the slides both show the top scored track. The same resolver they
  // use, so all three say the same thing about one review.
  const sel = resolveSelections(r.selections, r.scores, r.album?.tracks)
  ...
  return {
    ...
    criteria: (r.criteriaModel?.length ? r.criteriaModel : DEFAULT_CRITERIA)
      .map(({ key, label }) => ({
        key, label,
        value: criterionValue(r.criteria?.[key])
      }))
...
export const publicReview = r => ({
  ...
  criteria: (r.criteriaModel?.length ? r.criteriaModel : DEFAULT_CRITERIA).map(({ key, label }) => ({
    key, label, value: criterionValue(r.criteria?.[key])
  })),
...
export function raterStats (reviews) {
  ...
  // The ladder most of this rater's work is on, so an average that spans two
  // scales is still printed to a precision one of them actually chose. Ties go
  // to whichever was seen first, which is the most recent by the order these
  // arrive in.
  const ladders = new Map()
  for (const r of reviews) {
    if (!r.scaleModel) continue
    const k = `${r.scaleModel.id}|${r.scaleModel.max}|${r.scaleModel.name}`
    const seen = ladders.get(k)
    if (seen) seen.n++
    else ladders.set(k, { scale: r.scaleModel, n: 1 })
  }
  const commonest = [...ladders.values()].sort((a, b) => b.n - a.n)[0]?.scale ?? null
...
export const publicComment = (c, author) => ({
  ...
  author: author
    ? { handle: author.handle, name: author.name || author.handle, image: author.image || null,
        verified: isVerified(author.email) }
    : { handle: null, name: 'Deleted account', image: null, verified: false }
})
