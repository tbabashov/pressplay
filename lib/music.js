...
import { featuresByTitle as discogsFeatures } from './discogs-credits.js'
...
export async function getAlbum (id) {
  ...
  // Deezer credits nobody on a lot of its catalogue, so two others fill in.
  //
  // Apple has the label's own credit line, but only where the label put it in
  // the track name. Where a record lists its guests in the metadata beside the
  // song instead, Apple's name is bare and there is nothing to read: ASTROWORLD
  // is uncredited there from end to end. Discogs keeps them as a field of their
  // own, so it answers for exactly the tracks Apple cannot.
  //
  // Asked together rather than one after the other: they are independent
  // lookups on the same two strings, and in sequence a record with a thin
  // Deezer entry waited for both in turn. Apple fills first where both have an
  // answer, because a track name is published by the label and a Discogs
  // submission is typed by a person.
  const bare = () => tracks.filter(t => !t.features.length)

  if (bare().length) {
    const [apple, discogs] = await Promise.all([
      // Only when Deezer said nothing at all: a record it did credit is
      // already right, and this would be a second network call for nothing.
      tracks.some(t => t.features.length)
        ? {}
        : withTimeout(featuresByTitle(a.artist?.name, a.title, mains), 4000, {}),
      withTimeout(discogsFeatures(a.artist?.name, a.title, artists), 6000, {})
    ])

    for (const t of tracks) {
      const guests = apple[titleKey(t.title)]
      if (guests?.length) t.features = guests
    }
    for (const t of bare()) {
      const guests = discogs[titleKey(t.title)]
      if (guests?.length) t.features = guests
    }
  }
