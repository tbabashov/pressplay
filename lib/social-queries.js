import { listPublishedReviews,
  listReviews, listReviewsLite, listAllPublishedLite, listProfiles, getProfile, listFollowing, listFollowers,
  countComments, listComments, voteTotals, myVotes, reviewId
} from './db/index.js'
import { rank } from './standings.js'
import { publicCard, publicPost, raterStats, publicComment } from './social-shape.js'
...
export async function raters ({ excludeEmail } = {}) {
  // One query for the reviews, not one per account. This asked the database for
  // a rater's whole published library once for every account that had one, so
  // the cost grew with the number of people using the site: at 28 publishing
  // accounts it was 29 round trips and about twelve seconds, past the ten a
  // Vercel function is given, and /browse answered 500 to everybody.
  //
  // Same rows, same grouping, done here instead of by asking again.
  const [profiles, all] = await Promise.all([listProfiles(), listAllPublishedLite()])

  const byEmail = new Map()
  for (const r of published(all)) {
    const list = byEmail.get(r.userEmail)
    if (list) list.push(r)
    else byEmail.set(r.userEmail, [r])
  }

  const rows = profiles.map(p => {
    const live = byEmail.get(p.email) ?? []
...
// How far back popularity looks. Ranking on votes needs the votes, which are a
// separate table, so a bounded window of recent ratings is scored rather than
// the whole history. It is also the better answer: an all time chart on a young
// site is the same five records forever.
const POPULAR_WINDOW = 300

export async function discover ({ sort = 'recent', viewerEmail = null, limit = 60, offset = 0 } = {}) {
  // One query for the page being shown, not every review in the database.
  // This used to list every account, read every review each of them had, build
  // the lot in memory and keep sixty, which is the whole table on every view.
  const take = sort === 'popular' ? POPULAR_WINDOW : limit
  const reviews = await listPublishedReviews({ limit: take, offset: sort === 'popular' ? 0 : offset })

  // The authors, once each, rather than once per rating.
  const authors = new Map()
  await Promise.all([...new Set(reviews.map(r => r.userEmail))].map(async email => {
    const p = await getProfile(email)
    if (p) authors.set(email, p)
  }))

  const rows = reviews.flatMap(r => {
    const p = authors.get(r.userEmail)
    if (!p) return []
    return [{ ...publicPost(r), id: reviewId(r.userEmail, r.albumId), by: publicProfile(p) }]
  })
  ...
  // Recent is already in order and already the right length; popular has just
  // been ranked across its window and is cut here.
  return sort === 'popular' ? full.slice(offset, offset + limit) : full
}
...
export async function followedFeed ({ viewerEmail, limit = 60 } = {}) {
  ...
  const rows = (await Promise.all(targets.map(async t => {
    ...
    return published(reviews).map(r => ({
      ...publicPost(r), id: reviewId(t, r.albumId), by: publicProfile(profile)
    }))
  }))).flat()
