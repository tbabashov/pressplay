...
// The same ladder with the gradients taken out, for the app's own chips.
//
// The signature 10 and 11 are diagonal gradients that run light, dark, light.
// Across a slide that is a sheen. On a chip the size of a thumbnail both light
// ends land on the edges and read as a border someone drew round it. The
// exported frames keep the gradient they were signed off with; every chip in
// the app gets the tier as one colour.
export function chipColour (score, scale) {
  const c = ratingColor(score, scale)
  return c.flat ? { ...c, bg: c.flat } : c
}
...
function signatureColor (score) {
  if (score === null || score === undefined) return { bg: 'rgba(120,120,128,0.24)', fg: '#a1a1a6' }
  if (score === NA) return { bg: 'rgba(142,142,147,0.5)', fg: '#e8e8ed' }
  // flat is the same tier as one colour, for anywhere the gradient cannot go.
  if (score >= 11) return { bg: 'linear-gradient(135deg, #fbcfe8 0%, #f472b6 42%, #db2777 78%, #f9a8d4 100%)', flat: '#db2777', fg: '#fff', glow: 'rgba(244,114,182,0.6)' }
  if (score >= 10) return { bg: 'linear-gradient(135deg, #7dd3fc 0%, #2563eb 55%, #38bdf8 100%)', flat: '#2563eb', fg: '#fff', glow: 'rgba(56,189,248,0.55)' }
