...
import { tierFor, rampColour, readableOn, tierBackground } from './scales.js'
...
// The same ladder, with a gradient that does not draw its own edge.
//
// The signature 10 and 11 run light, dark, light along a diagonal. Across a
// slide that is a sheen. On a chip the size of a thumbnail the two light ends
// land in opposite corners and read as a border someone drew round it. The
// gradient stays, it just runs one way and top to bottom, so it shades instead
// of outlining. The exported frames keep the diagonal they were signed off
// with, where there is room for it to read as intended.
export function chipColour (score, scale) {
  const c = ratingColor(score, scale)
  return c.chip ? { ...c, bg: c.chip } : c
}
...
export function ratingColor (score, scale) {
  if (scale && !scale.signature) {
    ...
    // A rung given two colours is a gradient. Readability is judged on the
    // first one, which is the colour the text actually sits on at the top.
    return { bg: tierBackground(tier) || hex, fg: readableOn(hex) }
  }
...
function signatureColor (score) {
  if (score === null || score === undefined) return { bg: 'rgba(120,120,128,0.24)', fg: '#a1a1a6' }
  if (score === NA) return { bg: 'rgba(142,142,147,0.5)', fg: '#e8e8ed' }
  // chip is the same tier shaded one way, for a box too small to carry the
  // diagonal without the light ends reading as an outline round it.
  if (score >= 11) return { bg: 'linear-gradient(135deg, #fbcfe8 0%, #f472b6 42%, #db2777 78%, #f9a8d4 100%)', chip: 'linear-gradient(180deg, #f472b6 0%, #db2777 100%)', fg: '#fff', glow: 'rgba(244,114,182,0.6)' }
  if (score >= 10) return { bg: 'linear-gradient(135deg, #7dd3fc 0%, #2563eb 55%, #38bdf8 100%)', chip: 'linear-gradient(180deg, #38bdf8 0%, #2563eb 100%)', fg: '#fff', glow: 'rgba(56,189,248,0.55)' }
