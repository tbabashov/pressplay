...
// The ladder a new account starts on. Picked by id rather than by position, so
// reordering the presets cannot quietly change what everybody gets.
//
// This was the eleven and is now the ten. The eleven is still here and is still
// the ladder the exported slides were designed around; it is just no longer
// what somebody is handed before they have chosen anything. Accounts already on
// it keep it: moving the top of the scale under a library that has been scored
// against it would reinterpret every rating in it at once.
export const DEFAULT_SCALE = SCALE_PRESETS.find(s => s.id === 'ten')
export const LEGACY_SCALE_ID = 'eleven'
export const SCALE_MAX_CEILING = 100
export const TIER_NAME_MAX = 18

// How many decimal places a score is shown to. A property of the scale rather
// than of the account, for the same reason the tiers are: a scale is stored on
// every review it was used for, so a rating keeps the precision it was given
// even after the ladder underneath it changes. Three is the ceiling because a
// fourth decimal on an average of six numbers is noise being presented as
// precision.
export const DECIMALS_MAX = 3
export const DEFAULT_DECIMALS = 1

export const normaliseDecimals = v => {
  const n = Math.round(Number(v))
  return Number.isFinite(n) && n >= 0 && n <= DECIMALS_MAX ? n : DEFAULT_DECIMALS
}
...
// A rung's background: one colour, or two shaded into each other. Top to bottom
// rather than corner to corner, because a diagonal on a box the size of a chip
// puts its two ends in opposite corners and reads as an outline round it.
export const tierBackground = tier => {
  const a = tier?.colour
  if (!a) return null
  return tier.colour2 ? `linear-gradient(180deg, ${a} 0%, ${tier.colour2} 100%)` : a
}
...
export function normaliseScale (scale) {
  ...
  const tiers = (Array.isArray(scale.tiers) ? scale.tiers : [])
    .map(t => ({
      ...
      colour: hex(t?.colour),
      // A second colour turns the rung into a gradient. Null means one flat
      // colour, which is what almost every rung is.
      colour2: hex(t?.colour2)
    }))
    .filter(t => {
  ...
  return {
    ...
    max,
    na: scale.na !== false,
    decimals: normaliseDecimals(scale.decimals),
...
// One place that turns a number into the string a screen shows.
//
// Takes the scale so the rating's own precision travels with it: a review rated
// on a ladder set to two decimals reads 8.44 on the leaderboard, in the feed, on
// its public page and on the slide, without each of those deciding separately.
// Without a scale it is one decimal, which is what every call site hardcoded
// before there was a setting.
export const fmtScore = (v, scale) =>
  (typeof v === 'number' ? v.toFixed(scale ? normaliseDecimals(scale.decimals) : DEFAULT_DECIMALS) : '—')
...
const mix = (hex, towardWhite, amount) => {
  const m = /^#?([0-9a-f]{6})$/i.exec(String(hex || ''))
  if (!m) return hex
  const ch = [0, 2, 4].map(i => parseInt(m[1].slice(i, i + 2), 16))
  const to = towardWhite ? 255 : 0
  return '#' + ch.map(c => Math.round(c + (to - c) * amount).toString(16).padStart(2, '0')).join('')
}

// A tier colour made readable as type on a given paper. The rating colours are
// fills; three styles paint the figure itself in them, where they need contrast.
// Keeps the hue, returns anything already legible untouched.
export function readableInk (hex, ink, target = 5) {
  const m = /^#?([0-9a-f]{6})$/i.exec(String(hex || ''))
  if (!m) return hex
  // Dark ink means a light page, and the safe direction is darker.
  const paperIsLight = LUM(ink) < 0.5
  const paper = paperIsLight ? '#ffffff' : '#000000'
  if (CONTRAST(hex, paper) >= target) return hex
  let out = hex
  for (let step = 1; step <= 20; step++) {
    out = mix(hex, !paperIsLight, step / 20)
    if (CONTRAST(out, paper) >= target) return out
  }
  return out
}
