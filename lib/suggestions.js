...
const PER_ARTIST = 2
// Drawn from a wider pool than it uses. Always taking the top four artists and
// their first two albums makes the strip identical on every visit, so a shuffle
// button would return the same twelve records and look broken. The pool is
// still the ones you rated highest; which of them come up is what changes.
const SEED_POOL = 10
const ALBUM_POOL = 6
...
const SIDE_PROJECT = /anniversar|deluxe|remaster|edition|reissue|live|instrumental|karaoke|greatest hits|best of|the remixes|director's cut|\bx\b|ensemble|orchestra|symphon|soundtrack|original score|\bost\b|motion picture|netflix|season \d|\bep\b|single|commentary|music from|inspired by|mixtape by|dj mix/i
...
const shuffleBy = (list, seed) => {
  // Deterministic for a given seed. The seed is drawn once per render on the
  // server, so the markup the browser receives already has an order and there
...
export async function suggestionsFor (reviews = [], { limit = 12, seed = 0, popular = [] } = {}) {
  ...
  const seeds = shuffleBy(
    [...byArtist.values()].sort((a, b) => (b.best - a.best) || (b.n - a.n)).slice(0, SEED_POOL),
    seed
  ...
  const own = await Promise.all(seeds.map(async s => {
    ...
    return shuffleBy(eligible.slice(0, ALBUM_POOL), seed)
      .slice(0, PER_ARTIST)
      .map(a => ({
  ...
  const near = await Promise.all(seeds.slice(0, RELATED_SEEDS).map(async s => {
    const others = shuffleBy(
      (await relatedArtists(s.artist).catch(() => []))
