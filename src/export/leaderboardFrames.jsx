  ...
  return (
    <FrameShell palette={palette} theme={theme}>
      <div style={{ marginBottom: 46 }}>
        <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: 6, textTransform: 'uppercase', color: palette.accent }}>
        ...
        <div style={{ fontSize: 30, fontWeight: 600, color: 'rgba(255,255,255,0.6)', marginTop: 16, lineHeight: 1.35 }}>
          ...
          <Surface key={i} theme={theme} radius={32} style={{ padding: '30px 34px', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 20 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 27, fontWeight: 500, lineHeight: 1.38, color: 'rgba(255,255,255,0.78)', marginTop: 10 }}>
                  {note.body}
                </div>
              </div>
            </div>
  ...
  return (
    <FrameShell palette={palette} theme={theme}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', textAlign: 'center' }}>
        <div style={{ fontSize: 32, fontWeight: 800, letterSpacing: 7, textTransform: 'uppercase', color: palette.accent }}>
          After the update
        </div>
        <div style={{ fontSize: 108, fontWeight: 800, letterSpacing: -3.5, marginTop: 16, lineHeight: 1.02 }}>
          The Full<br />Leaderboard
        </div>
        <div style={{ fontSize: 36, fontWeight: 600, color: 'rgba(255,255,255,0.6)', marginTop: 22 }}>
  ...
  return (
    <FrameShell palette={palette} theme={theme}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 26 }}>
        <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: 5, textTransform: 'uppercase', color: palette.accent }}>
          Leaderboard
        </div>
        <div style={{ fontSize: 27, fontWeight: 700, color: 'rgba(255,255,255,0.55)', fontVariantNumeric: 'tabular-nums' }}>
          {from}–{to} of {total}
        </div>
      </div>
          ...
          {rows.map((r, i) => (
            <div key={r.albumId} style={{
              ...
              <span style={{
                ...
                fontSize: 34, fontWeight: 800, color: 'rgba(255,255,255,0.85)',
                fontVariantNumeric: 'tabular-nums'
              }}>
                {r.rank}
              </span>
          ...
          {jumps.map((j, i) => (
            <div key={`${j.albumId}:${j.name}`} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
                ...
                <span style={{ fontSize: 30, fontWeight: 800, color: 'rgba(255,255,255,0.45)' }}>→</span>
...
export function MoversFrame ({ climbers, fallers, palette, theme }) {
  const Col = ({ title, tone, rows }) => (
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 24, fontWeight: 800, letterSpacing: 3, textTransform: 'uppercase', color: tone, marginBottom: 14 }}>
        {title}
      </div>
        ...
        {rows.map((r, i) => (
          <div key={r.albumId} style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 600, color: 'rgba(255,255,255,0.5)', marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>
