...
import { ratingColor, scoreText, SCALE_ROWS, fmtRuntime, fmtDuration } from '../colors.js'
import { NA, fmtScore } from '../rating.js'
import {
...
const DOME = {
  background: [
    'radial-gradient(120% 80% at 50% 0%, rgba(255,255,255,0.30) 0%, rgba(255,255,255,0.10) 42%, rgba(255,255,255,0.03) 72%)',
    'linear-gradient(180deg, rgba(255,255,255,0.16) 0%, rgba(255,255,255,0.06) 55%, rgba(255,255,255,0.02) 100%)'
  ].join(', '),
  border: '3px solid rgba(255,255,255,0.42)',
  borderBottom: 'none',
  boxShadow: [
    'inset 0 3px 0 rgba(255,255,255,0.6)',
    'inset 0 0 120px rgba(255,255,255,0.10)',
          ...
          {CORNERS.map(([fx, fy]) => (
            <div
              ...
              style={{
                ...
                background: '#fff', border: '5px solid rgba(0,0,0,0.55)',
  ...
  return (
    <FrameShell palette={palette} theme={theme} fullBleed>
      ...
      <div style={{
        ...
        <div style={{
          fontSize: 50, fontWeight: 800, letterSpacing: 9, textTransform: 'uppercase',
        ...
        <FitText
          ...
          style={{ color: 'rgba(255,255,255,0.78)', marginTop: 10, width: 880, textAlign: 'center' }}
        >
          {album.artists.join(', ')}
        </FitText>
...
function Header ({ album, palette, theme, subtitle }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 32, marginBottom: 34 }}>
      <img src={album.coverProxied} alt="" style={{ width: 168, height: 168, borderRadius: 26, objectFit: 'cover', boxShadow: '0 20px 60px rgba(0,0,0,0.55)', flexShrink: 0 }} />
      <div style={{ flex: 1, minWidth: 0 }}>
        <FitText size={48} min={30} weight={800} fitKey={album.name} style={{ letterSpacing: -1 }}>
          {album.name}
        </FitText>
        <FitText size={32} min={22} weight={600} fitKey={album.artists.join(', ')} style={{ color: 'rgba(255,255,255,0.72)', marginTop: 8 }}>
          {album.artists.join(', ')}
        </FitText>
        ...
        return (
          <div key={s} style={{ minWidth: 0 }}>
            ...
            <div
              ...
              style={{
                fontSize: labelSize, fontWeight: 600, color: 'rgba(255,255,255,0.65)',
...
function TrackRows ({ tracks, ratings, rowH, baseSize, drop, chip, theme }) {
  ...
  return tracks.map((t, i) => (
    <div key={t.id} style={{
      display: 'flex', alignItems: 'center', gap: 20, height: rowH,
      borderBottom: i < tracks.length - 1 ? rowRule(theme) : 'none'
    }}>
      <span style={{
        width: 42, textAlign: 'right', fontSize: fs - 6, flexShrink: 0,
        color: 'rgba(255,255,255,0.42)', fontWeight: 600, fontVariantNumeric: 'tabular-nums'
      }}>
        {t.trackNumber}
      </span>
      <div
        ...
        style={{
          flex: 1, minWidth: 0, fontSize: fs, fontWeight: 600,
      ...
      >
        ...
        {t.features.length > 0 && (
          <span
            ...
            style={{ fontSize: fs - drop, fontWeight: 500, color: 'rgba(255,255,255,0.48)' }}
          >
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        background: theme?.glass ? 'rgba(255,255,255,0.14)' : 'rgba(255,255,255,0.09)',
        overflow: 'hidden'
      }}>
        <div style={{
      ...
      <span style={{
        ...
        fontVariantNumeric: 'tabular-nums', color: has ? '#fff' : 'rgba(255,255,255,0.35)',
        flexShrink: 0
      }}>
  ...
  return (
    <div style={{ ...surfaceStyle({ glass: true }, { radius: 34 }), padding: '24px 30px 26px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 17, fontWeight: 800, letterSpacing: 2.4, textTransform: 'uppercase', color: palette.accent, marginBottom: 7 }}>
            Now Playing
          </div>
          ...
          <FitText
            ...
            style={{ color: 'rgba(255,255,255,0.62)', marginTop: 4 }}
          >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 20 }}>
        ...
        {dur > 0 && (
          <span style={{ fontSize: 19, fontWeight: 600, color: 'rgba(255,255,255,0.6)', fontVariantNumeric: 'tabular-nums', width: 62 }}>
            {fmtDuration(at)}
          </span>
        )}
        <div style={{ flex: 1, height: 8, borderRadius: 4, background: 'rgba(255,255,255,0.2)', position: 'relative' }}>
          <div style={{ width: '38%', height: '100%', borderRadius: 4, background: 'rgba(255,255,255,0.92)' }} />
          <div style={{
            position: 'absolute', left: '38%', top: '50%', width: 20, height: 20,
            marginLeft: -10, marginTop: -10, borderRadius: '50%', background: '#fff',
            boxShadow: '0 3px 10px rgba(0,0,0,0.45)'
          }} />
        </div>
        {dur > 0 && (
          <span style={{ fontSize: 19, fontWeight: 600, color: 'rgba(255,255,255,0.6)', fontVariantNumeric: 'tabular-nums', width: 62, textAlign: 'right' }}>
...
function Superlative ({ label, value, tone, theme }) {
  return (
    <Surface theme={theme} radius={26} style={{ padding: '18px 24px', flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: 2, textTransform: 'uppercase', color: tone }}>{label}</div>
    ...
    <FrameShell palette={palette} theme={theme}>
      ...
      <Surface theme={theme} radius={36} style={{
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 26, fontWeight: 800, letterSpacing: 4.5, textTransform: 'uppercase', color: palette.accent }}>
            Final Rating
          </div>
          <div style={{ fontSize: 40, fontWeight: 700, color: 'rgba(255,255,255,0.82)', marginTop: 6 }}>{tier}</div>
        </div>
        <span style={{
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        color: highlight ? palette.accent : 'rgba(255,255,255,0.55)',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div
          ref={artistRef}
          style={{
            fontSize: artistSize, fontWeight: 500, color: 'rgba(255,255,255,0.6)',
    ...
    <FrameShell palette={palette} theme={theme}>
      <div style={{ textAlign: 'center', marginBottom: 44 }}>
        ...
        <div style={{ fontSize: 86, fontWeight: 800, letterSpacing: -2, marginTop: 12 }}>
          #{rank} <span style={{ fontSize: 44, fontWeight: 600, color: 'rgba(255,255,255,0.5)' }}>of {totalRanked} albums</span>
        </div>
      </div>

      <Fill theme={theme} gap={16}>
        {ladder.map((entry, i) => entry.gap ? (
          <div key={`gap-${i}`} style={{ textAlign: 'center', fontSize: 40, fontWeight: 800, letterSpacing: 14, color: 'rgba(255,255,255,0.35)', lineHeight: '30px' }}>
            ···
          </div>
        ) : (
          <RankRow
  ...
  return (
    <div style={{ width: CELL }}>
      <div style={{
        ...
        border: isCurrent ? `4px solid ${palette.accent}` : rated ? 'none' : '3px dashed rgba(255,255,255,0.32)',
        background: rated ? 'transparent' : 'rgba(255,255,255,0.05)'
      }}>
        {album.cover && (
          <img
            src={album.cover} alt=""
        ...
        {!rated && (
          <div style={{
            ...
            <span style={{
              ...
              fontSize: 52, fontWeight: 800, color: 'rgba(255,255,255,0.85)'
            }}>?</span>
          </div>
        )}
        {rated && (
      ...
      <FitText
        ...
        style={{ marginTop: 10, color: rated ? '#fff' : 'rgba(255,255,255,0.55)', textAlign: 'center' }}
      >
        {album.name}
      </FitText>
      <div style={{
        fontSize: 20, fontWeight: 600, marginTop: 2, textAlign: 'center',
        color: rated ? palette.accent : 'rgba(255,255,255,0.38)'
      }}>
...
export function DiscographyFrame ({ group, page, pages, currentAlbumName, palette, theme }) {
  const ratedCount = group.albums.filter(a => a.rated).length
  return (
    <FrameShell palette={palette} theme={theme}>
      <div style={{ textAlign: 'center', marginBottom: 38 }}>
        ...
        <div style={{ fontSize: 26, fontWeight: 600, color: 'rgba(255,255,255,0.55)', marginTop: 8 }}>
          {ratedCount} of {group.albums.length} rated
        </div>
      </div>
